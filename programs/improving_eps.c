...
int main(int argc, char *argv[]){
	...
	//we have to rename drivers so that there are no zero lines in A
	int *rename_driver = new int[n];
	bool *should_be_nan = new bool[n];
	int offset = 0;
	for(int i = 0; i < n; ++i){
		rename_driver[i] = i-offset;
		should_be_nan[i] = 0;
		if(st_cap[i] == 0 || i == observed){
			offset++; //if unit i contributes no spikes or if it is the observed uni
			should_be_nan[i] = 1;
		}
	}
	//and now to go through the intervals, and at the same time fill A and b
	VectorXf r(n-offset); //result
	double largest = 0;
	if(st[observed]-1 > n-offset){ //if there are enough interspike intervals for an overdetermined system
		MatrixXf A(st[observed]-1-1, n-offset); //sepravi vzamem vse intervale, pa omege ne fitam
		...
		for(int in = 0; in < st[observed]-1-1; ++in){
			...
			//now lets fill A
			for(int i = 0; i < n-offset; ++i){ //here its n-1 because every observed one has n-1 connections
				A(in,i) = 0;
			}
			for(int s = 0; s < spikes_ct; ++s){
				int which_driver = int(round(sspikes[s][0]));
				A(in, rename_driver[which_driver]) += PRC(prc, sspikes[s][2]);
			}
			//and lets fill b
			if(observed != 0) b(in) = 2*Pi - prc[0]*intervals[0][in][int(round(intervals[0][in][0]))+1];
			else b(in) = 2*Pi - prc[0]*intervals[1][in][int(round(intervals[1][in][0]))+1];
		}
		//and minimize
		...
		for(int i = 0; i < n-offset; ++i){
			if(fabs(r(i)) > largest) largest = fabs(r(i));
		}
	}
	else{
		printf("\t\tEPS: not enough intervals for an overdetermined system\n");
		for(int i = 0; i < n-offset; ++i){
	...
	f = fopen("EPS.txt","wt");
	for(int i = 0; i < n; ++i){
		if(should_be_nan[i]) fprintf(f, "%lf\n", 0.0/0.0);
		else{
			fprintf(f,"%lf\n", r(rename_driver[i])/largest);
		}
	}
	fclose(f);
